...
static void handle_factory_provision(cJSON *params)
{
    ...
    /* Thread credentials are NOT part of factory_provision in the
     * cloud-canonical architecture. They are generated by the cloud
     * `adopt_device` edge function at user adoption time (BLE flow) and
     * pushed to H2 via UART CMD_SET_CREDENTIALS. See
...
static void handle_customer_reset(cJSON *params)
{
     ...
     * - Clears S3 adoption namespace (device tokens, cached Thread creds,
     *   adoption_state)
    ...
    /* Tell H2 to wipe its Thread credentials and stop the Thread stack so
     * the device returns to the UNPROVISIONED state on both chips. Best
     * effort - we still reboot regardless. */
    if (h2_comm_is_connected()) {
        esp_err_t h2_err = h2_comm_clear_credentials(5000);
        if (h2_err != ESP_OK) {
            ESP_LOGW(TAG, "H2 clear_credentials failed during consumer_reset: %s",
