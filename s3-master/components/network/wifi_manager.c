...
esp_err_t wifi_init(void)
{
    ...
    /* Initialize TCP/IP stack (may already be initialized by main for coexistence) */
                             ...
                             int32_t event_id, void *event_data)
{
    if (event_id == IP_EVENT_STA_GOT_IP) {
        ...
        /* Set WiFi as the default netif for routing and DNS resolution.
         * This is critical when Thread is also running - without this,
         * DNS queries may fail because LwIP doesn't know which interface to use. */
        esp_netif_set_default_netif(s_wifi.netif);
        ESP_LOGI(TAG, "Set WiFi as default netif for routing/DNS");

        /* Configure DNS servers for reliable cloud connectivity.
         * Use well-known public DNS servers (Cloudflare and Google) instead of DHCP DNS.
         * This is more reliable with WiFi+Thread coexistence where radio time-sharing
         * can cause packet loss - public DNS servers respond faster than most router DNS. */
        ip_addr_t dns_primary, dns_secondary;
        IP_ADDR4(&dns_primary, 1, 1, 1, 1);      /* Cloudflare DNS - fast and reliable */
        IP_ADDR4(&dns_secondary, 8, 8, 8, 8);    /* Google DNS - reliable fallback */
        dns_setserver(0, &dns_primary);
        dns_setserver(1, &dns_secondary);
        ESP_LOGI(TAG, "DNS servers: 1.1.1.1 (primary), 8.8.8.8 (backup)");
