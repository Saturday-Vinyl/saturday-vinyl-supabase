...
/* DNS-based connectivity test URL (requires DNS resolution) */
#define DNS_TEST_URL            "https://www.google.com/generate_204"
...
esp_err_t http_test_connectivity(void)
{
    ...
    /* Test 1: IP-based HTTPS (no DNS needed) */
    ...
    if (err == ESP_OK) {
        ...
        ESP_LOGE(TAG, "Internet connectivity test failed (IP-based)");
        return err;
    }

    /* Test 2: DNS-based HTTPS (verifies DNS resolution works) */
    ESP_LOGI(TAG, "Testing DNS-based connectivity (google.com)...");
    err = http_get(DNS_TEST_URL, &response, 10000);  /* Longer timeout for DNS */

    if (err == ESP_OK) {
        /* Google returns 204 No Content for this URL */
        if (response.status_code == 204 || response.status_code == 200) {
            ESP_LOGI(TAG, "DNS-based connectivity OK (response in %lld ms)",
                     (long long)response.request_time_ms);
            http_response_free(&response);
            return ESP_OK;
        } else {
            ESP_LOGW(TAG, "DNS test unexpected status: %d", response.status_code);
            http_response_free(&response);
            return ESP_OK;  /* DNS worked, that's the main point */
        }
    } else {
        ESP_LOGE(TAG, "DNS-based connectivity FAILED - DNS resolution may be broken");
        return err;
    }
}
