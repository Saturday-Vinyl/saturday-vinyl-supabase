...
### Prompt 35: Global QR Scanning with Keyboard Prefix Detection

**Context:** Currently, QR scanning is only accessible from the unit detail screen via a button in the AppBar. USB barcode scanners work like keyboards, typing the scanned data into focused text fields. We need to make QR scanning globally accessible and add intelligent background detection for USB scanners configured with prefix characters.

**User Story:** As a production floor worker, I want to scan QR codes from anywhere in the app without navigating to a specific screen, and I want my USB scanner to automatically navigate me to the scanned production unit without any additional clicks.

**Prompt:**

```
We need to enhance our QR scanning system with two major improvements:

1. GLOBAL QR SCAN ACCESS
   - Add a persistent FAB (Floating Action Button) to MainScaffold
   - Button should be visible across all screens
   - Opens QR scan screen when clicked
   - Saturday brand styling
   - Works on both desktop and mobile

2. KEYBOARD PREFIX DETECTION (Desktop Only)
   - Implement background keyboard listener in MainScaffold
   - Detect two prefix options:
     a) Keyboard shortcut: Ctrl+Shift+Q
     b) Special character: § (section sign, ASCII 167)

   - SMART DELAY LOGIC:
     * When prefix detected, start 100ms delay timer
     * If additional characters received within 100ms → USB scanner detected
       → Continue buffering until Enter key pressed
       → Strip prefix character
       → Process as complete QR scan
       → Navigate directly to UnitDetailScreen
     * If NO additional characters within 100ms → Manual keyboard shortcut
       → Open QRScanScreen for manual scanning

   - Buffer management:
     * 100ms detection delay
     * Continue buffering until Enter key if scanner detected
     * 2-second overall timeout (clear buffer and reset)
     * Strip prefix character before processing
     * Show error SnackBar for invalid scans

3. SCANNER PROGRAMMING UI (Settings Screen)
   - Add new card: "USB Scanner Configuration"
   - Display step-by-step instructions
   - Step 1: "Enter Programming Mode"
     → QR code placeholder: [SCANNER_PROGRAMMING_MODE]
     → Helper text explaining scanner manual
   - Step 2: "Set Prefix Character"
     → Generate QR code for § character (section sign, ASCII 167)
     → Helper text: "Configures scanner to send § before each scan"
   - Step 3: "Exit Programming Mode"
     → QR code placeholder: [SCANNER_EXIT_PROGRAMMING_MODE]
     → Helper text for saving settings
   - Add "Test Scanner" button
     → Opens QR scan screen to verify configuration
     → Shows success message if prefix detected
   - Visual design:
     → Numbered steps (1, 2, 3)
     → QR codes inline (~150x150)
     → Copy buttons for raw QR data
     → Info icons with tooltips

4. UPDATE EXISTING COMPONENTS
   - MainScaffold: Add FAB + keyboard listener
   - QRScannerDesktop: Strip § prefix in _processInput()
   - UnitDetailScreen: Remove "Scan Next QR" button (no longer needed)
   - QRScannerService: Add stripPrefix() helper method

FILES TO CREATE:
- lib/services/keyboard_listener_service.dart
  * Encapsulate all keyboard event handling
  * Smart delay detection (100ms)
  * Buffer management
  * Callbacks: onScanDetected(String), onManualShortcut()

- lib/widgets/production/qr_fab_button.dart
  * Reusable FAB component
  * Saturday brand colors
  * QR scanner icon
  * Tooltip: "Scan QR Code (Ctrl+Shift+Q)"

- lib/widgets/settings/scanner_config_card.dart
  * Programming QR code generation
  * Step-by-step UI
  * Placeholder support for TBD codes
  * Test functionality

FILES TO UPDATE:
- lib/screens/main_scaffold.dart
  * Initialize keyboard listener (desktop only)
  * Add persistent FAB
  * Implement scan callbacks
  * Error handling (SnackBar)

- lib/widgets/production/qr_scanner_desktop.dart
  * Strip § prefix in _processInput()
  * Trim whitespace

- lib/services/qr_scanner_service.dart
  * Add stripPrefix() method
  * Update processScannedCode() to handle prefix

- lib/screens/settings/settings_screen.dart
  * Add ScannerConfigCard widget

- lib/screens/production/unit_detail_screen.dart
  * Remove "Scan Next QR" AppBar button
...
bool _handleKeyEvent(KeyEvent event) {
  if (event is KeyDownEvent) {
    ...
    // Buffer characters after prefix
...
void _onPrefixDetected() {
  ...
  // Start 100ms delay timer to detect scanner vs manual
  _delayTimer = Timer(Duration(milliseconds: 100), () {
    if (_scanBuffer.isEmpty) {
      // No characters → manual keyboard shortcut
      onManualShortcut?.call();
      _reset();
    }
    // If characters received, continue buffering until Enter
  });

  // Start overall 2s timeout
  _timeoutTimer = Timer(Duration(seconds: 2), _reset);
}

void _bufferCharacter(String char) {
  if (char == '\n' || char == '\r') {
    // Enter pressed - process scan
...
- [ ] FAB visible on all screens (desktop + mobile)
- [ ] FAB opens QR scan screen
- [ ] Ctrl+Shift+Q alone opens scan screen (manual mode)
- [ ] USB scanner with § prefix navigates to unit detail (scanner mode)
- [ ] 100ms delay correctly distinguishes scanner vs manual
- [ ] Enter key completes buffered scan
- [ ] Invalid scans show error SnackBar
- [ ] Buffer clears after 2s timeout
- [ ] § prefix stripped before processing
- [ ] Scanner config card displays in settings
- [ ] Programming QR codes generate correctly
- [ ] Test button works
- [ ] Keyboard listener only active on desktop
- [ ] Mobile FAB works with camera scanner

Please implement this complete solution with proper error handling and user feedback.
```

**Expected Outcome:**
- Persistent FAB on all screens for QR scanning
- Background keyboard listener detects USB scanner activity
- Smart 100ms delay distinguishes scanner from manual keyboard input
- USB scanners with § prefix navigate directly to unit detail
- Manual Ctrl+Shift+Q shortcut opens scan screen
- Settings screen has scanner programming UI with QR codes
- Seamless production floor workflow with no extra clicks

**Key Implementation Details:**

1. **KeyboardListenerService**:
   - Stateful service managing keyboard events
   - Smart delay timer (100ms) for scanner detection
   - Buffer management with 2s timeout
   - Two callbacks: onScanDetected and onManualShortcut

2. **QRFABButton**:
   - Material FAB with QR scanner icon
   - Saturday brand primary color
   - Tooltip shows keyboard shortcut
   - Positioned bottom-right

3. **ScannerConfigCard**:
   - Three-step programming flow
   - Inline QR code generation
   - Placeholders for scanner-specific codes
   - Test functionality to verify setup

4. **Smart Navigation**:
   - Scanner mode: Direct to unit detail (fastest)
   - Manual mode: Open scan screen (flexible)
   - Error handling: SnackBar with clear message

**Platform Considerations:**
- Desktop: Keyboard listener + FAB
- Mobile: FAB only (camera scanner)
- Web: FAB only (limited support)

**User Workflows:**

*Production Floor with USB Scanner:*
1. Worker scans QR code
2. Scanner sends: §https://app.saturday.com/unit/abc-123[Enter]
3. App detects prefix and characters within 100ms
4. Buffers until Enter
5. Navigates directly to unit detail
6. Worker completes step, scans next unit
7. Repeat seamlessly

*Ad-hoc Manual Scanning:*
1. User presses Ctrl+Shift+Q from any screen
2. No additional characters within 100ms
3. QR scan screen opens
4. User scans with USB scanner or enters manually

*One-time Scanner Setup:*
1. Admin opens Settings
2. Scans programming mode QR (placeholder)
3. Scans prefix character QR (§)
4. Scans exit programming QR (placeholder)
5. Tests with "Test Scanner" button
6. Success confirmation

**Prompt Number:** 35
**Dependencies:** Prompt 18 (QR Scanning), Prompt 17 (Unit Detail Screen)
