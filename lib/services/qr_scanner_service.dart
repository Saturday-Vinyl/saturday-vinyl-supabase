...
class QRScannerService {
  ...
  // Prefix character used by USB scanners
  static const prefixChar = '§';
  ...
  Future<String> processScannedCode(String scannedText) async {
    try {
      ...
      final cleanedText = stripPrefix(scannedText);

      // Use QRService to parse and validate
      final uuid = _qrService.parseQRCode(cleanedText);
  ...
  /// Strip prefix character from scanned text
  String stripPrefix(String text) {
    String cleaned = text.trim();

    // Remove § prefix if present
    if (cleaned.startsWith(prefixChar)) {
      cleaned = cleaned.substring(1).trim();
      AppLogger.debug('Stripped prefix character from scanned text');
    }
