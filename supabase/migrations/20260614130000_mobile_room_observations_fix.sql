-- ============================================================================
-- Migration: 20260614130000_mobile_room_observations_fix.sql
-- Project: saturday-mobile-app
-- Description: Fixes "column reference X is ambiguous" errors in
--              mobile_room_observation(). The RETURNS TABLE(...) clause
--              declares OUT parameters that become function-scope variables
--              and shadow CTE column names. Adds #variable_conflict
--              use_column so CTE columns win in any conflict.
-- Date: 2026-06-14
...
BEGIN
  ...
  temporal_qualified AS (
    SELECT * FROM temporal_candidates tc
    WHERE tc.days_ago BETWEEN 7  AND 14
       OR tc.days_ago BETWEEN 28 AND 35
       OR tc.days_ago BETWEEN 360 AND 370
  ),
  temporal AS (
    SELECT
  ...
  cratelist AS (
    SELECT
      ...
      CASE
        WHEN cc.last_play IS NULL THEN NULL
        ELSE EXTRACT(DAY FROM (now() - cc.last_play))::int
      END AS days_since_last_play,
      NULL::int AS play_count
    FROM cratelist_candidates cc
    ORDER BY random()
    LIMIT 1
  ),
