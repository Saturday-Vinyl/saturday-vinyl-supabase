...
-- Add constraint to validate machine_type (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'valid_machine_type' AND conrelid = 'public.machine_macros'::regclass
    ) THEN
...
-- Add constraint for positive execution_order (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'positive_execution_order' AND conrelid = 'public.machine_macros'::regclass
    ) THEN
        ALTER TABLE public.machine_macros
...
-- Add constraint to ensure name is not empty (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'non_empty_name' AND conrelid = 'public.machine_macros'::regclass
    ) THEN
        ALTER TABLE public.machine_macros
...
-- Add constraint to ensure gcode_commands is not empty (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'non_empty_gcode' AND conrelid = 'public.machine_macros'::regclass
    ) THEN
        ALTER TABLE public.machine_macros
...
-- Add constraint to ensure icon_name is not empty (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'non_empty_icon' AND conrelid = 'public.machine_macros'::regclass
    ) THEN
        ALTER TABLE public.machine_macros
...
CREATE INDEX IF NOT EXISTS idx_machine_macros_machine_type_active
ON public.machine_macros(machine_type, is_active);

-- Create index on execution_order for sorting
CREATE INDEX IF NOT EXISTS idx_machine_macros_execution_order
...
DROP POLICY IF EXISTS machine_macros_select_policy ON public.machine_macros;
DROP POLICY IF EXISTS machine_macros_insert_policy ON public.machine_macros;
DROP POLICY IF EXISTS machine_macros_update_policy ON public.machine_macros;
DROP POLICY IF EXISTS machine_macros_delete_policy ON public.machine_macros;
...
-- Create trigger to call the function (drop and recreate for idempotency)
DROP TRIGGER IF EXISTS machine_macros_updated_at_trigger ON public.machine_macros;
...
-- Insert sample macros for CNC (only if table is empty to avoid duplicates)
INSERT INTO public.machine_macros (name, description, machine_type, icon_name, gcode_commands, execution_order)
SELECT * FROM (VALUES
...
) AS v(name, description, machine_type, icon_name, gcode_commands, execution_order)
WHERE NOT EXISTS (SELECT 1 FROM public.machine_macros WHERE machine_type = 'cnc' LIMIT 1);

-- Insert sample macros for Laser (only if table is empty to avoid duplicates)
INSERT INTO public.machine_macros (name, description, machine_type, icon_name, gcode_commands, execution_order)
SELECT * FROM (VALUES
...
WHERE NOT EXISTS (SELECT 1 FROM public.machine_macros WHERE machine_type = 'laser' LIMIT 1);
