...
esp_err_t thread_br_start(void)
{
    ...
    /* Get Wi-Fi netif for backbone interface */
    esp_netif_t *wifi_netif = wifi_get_netif();
    if (wifi_netif == NULL) {
        ESP_LOGW(TAG, "Wi-Fi netif not available - border routing disabled");
        ESP_LOGI(TAG, "Thread mesh will work but without IP bridging");
    } else {
        /* Set Wi-Fi as the backbone interface for border routing */
        ESP_LOGI(TAG, "Setting Wi-Fi as backbone interface for border routing");
        esp_openthread_set_backbone_netif(wifi_netif);

        /* Initialize Border Router functionality */
        ret = esp_openthread_border_router_init();

        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "Border router init failed: %s - continuing without border routing",
                     esp_err_to_name(ret));
            /* Continue anyway - Thread mesh still works */
        } else {
            ESP_LOGI(TAG, "Border Router initialized successfully");
        }
    }
