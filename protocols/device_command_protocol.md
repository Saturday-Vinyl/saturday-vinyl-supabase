...
6. [Device Adoption Lifecycle](#device-adoption-lifecycle)
...
#### Mesh Relay Routing (Crate and other Hub-relayed devices)

Devices that lack direct cloud connectivity (Crates, etc.) reach the cloud through a Hub on the user's account. Since firmware v0.6.0 a user account may have **multiple Hubs sharing one Thread mesh**, and a Crate may parent through any of them. The cloud cannot reliably know which Hub currently has a working CoAP path to a given Crate, so it broadcasts the command to all of them and lets the Hubs decide.

**Routing rules (`broadcast_device_command()` trigger):**

| Target device | Routing |
|---|---|
| `devices.hub_mac_address IS NULL` (Hub itself, or device with direct cloud access) | Send to `device:{mac_address}` — the device's own channel. |
| `devices.hub_mac_address IS NOT NULL` (Crate / Thread-only device) | Fan out: send to `device:{hub_mac}` for **every** Hub on the same `units.consumer_user_id` account, with `target_mac` set to the Crate's MAC. |

`devices.hub_mac_address` is still maintained by the heartbeat trigger but is now treated as informational "last reachable via" telemetry, not as the routing key.

**Hub behavior on receiving a `target_mac` command:**

1. If the target MAC is in the Hub's local Crate cache, relay via CoAP to the mesh node. The Crate's `command_ack` / `command_result` heartbeats flow back through this Hub.
2. If the target MAC is **not** in the local cache, **silently drop the command**. Do not execute it locally (Crate-targeted commands must never run on the Hub), and do not publish a `command_result` failure (a sibling Hub on the same account may successfully relay).

**Idempotency requirement.** Because two or more Hubs may both have a Crate cached during mesh transitions, the same `target_mac` command may be relayed and executed more than once. Relay-eligible commands must therefore be idempotent (`scan`, `get_status`, `consumer_reset`, `reboot`, etc.). Any future non-idempotent command must use a delivery path other than the mesh relay broadcast.
...
> **Removed in v1.4.0:** Thread credential fields (`thread_network_name`,
> `thread_channel`, `thread_pan_id`, `thread_network_key`) are no longer
> accepted at factory provisioning. Thread credentials are now generated by
> the cloud at user adoption time and pushed to the H2 over UART. See
> [Device Adoption Lifecycle](#device-adoption-lifecycle).
...
> **Removed in v1.4.0:** `thread_credentials` is no longer part of the
> `factory_provision` response. Thread credentials are issued by the cloud
> `adopt_device` edge function at user adoption time, not at factory
> provisioning. See [Device Adoption Lifecycle](#device-adoption-lifecycle).
...
## Device Adoption Lifecycle

A Hub goes through two distinct provisioning phases:

1. **Factory provisioning** (this protocol, `factory_provision`) — assigns
   identity (`serial_number`, MAC linkage in cloud, `cloud_url`, `anon_key`,
   factory Wi-Fi). Happens once, at the factory. Does **not** establish a
   Thread network.
2. **User adoption** (BLE flow, see [BLE Provisioning Protocol](ble_provisioning_protocol.md))
   — a signed-in user claims the Hub for their Saturday account. The cloud
   `adopt_device` edge function generates or fetches the user's
   account-scoped Thread credentials and returns them along with Hub-scoped
   access/refresh tokens.

### Why Thread credentials are not part of factory provisioning

Thread credentials define a Thread mesh. Putting per-Hub credentials in at
the factory means each Hub forms a separate mesh; the user cannot have
multiple Hubs in one home unless they share credentials. The cloud-canonical
architecture solves this by issuing a single credential set per user account
and having every Hub the user adopts join that one mesh.

### Cloud-side endpoints (Supabase Edge Functions)

| Endpoint | Method | Caller | Auth | Purpose |
|---|---|---|---|---|
| `/functions/v1/adopt_device` | POST | Hub or app | User JWT | Claim ownership, ensure user's `thread_networks` row, optionally issue device-scoped tokens. Replaces the older `claim-unit` function. |
| `/functions/v1/get_thread_credentials` | GET | Hub or app | User JWT or device access token | Fetch the user's current Thread credentials. |
| `/functions/v1/refresh_device_session` | POST | Hub | (refresh token in body) | Rotate the device's access/refresh token pair. |
| `/functions/v1/unadopt_device` | POST | App | User JWT | Release device from user account, revoke its session. |

The Hub uses its own **device session tokens** (one access + one refresh,
returned by `adopt_device`) for authenticated cloud calls after adoption.
The user's JWT is conveyed over BLE once and immediately discarded by the
Hub after the `adopt_device` round-trip.

### S3 ↔ H2 UART surface (new in v1.4.0)

The S3 master pushes Thread credentials to the H2 coprocessor via these
binary UART commands, defined in `shared/include/s3_h2_protocol.h`:

| Command | Direction | Payload | Behavior |
|---|---|---|---|
| `CMD_SET_CREDENTIALS` (0x0A) | S3 → H2 | `s3h2_credentials_payload_t` | Persist to H2 NVS, (re)start Thread stack with these credentials. |
| `CMD_CLEAR_CREDENTIALS` (0x0B) | S3 → H2 | empty | Wipe H2 Thread NVS, stop Thread, return to `UNPROVISIONED`. |

H2 reports a new state value `THREAD_STATE_UNPROVISIONED (0x06)` for the
idle-no-credentials condition. **The H2 never generates Thread credentials
locally** — it idles in `UNPROVISIONED` until the S3 pushes a set.

### Hub boot reconciliation

On every boot the S3 reads its NVS-cached Thread credentials and pushes them
to the H2 via `CMD_SET_CREDENTIALS`. This is idempotent if H2 already has
them, and provides a self-healing path if H2 NVS gets wiped (regression,
manual erase, future bug). The S3 cache is populated at adoption time and
treated as authoritative until replaced by a subsequent adoption.

### Consumer reset

`consumer_reset` clears the user-side state on both chips:

- S3: Wi-Fi credentials, device tokens, cached Thread credentials, adoption
  state (preserves factory cloud config).
- H2: receives `CMD_CLEAR_CREDENTIALS` over UART, wipes its Thread NVS, and
  returns to `UNPROVISIONED`.

After this command the Hub returns to BLE provisioning advertising, ready
for a fresh user adoption.
