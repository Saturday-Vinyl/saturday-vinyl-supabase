...
**Cloud Storage:** The admin app must merge the input params (`factory_input` fields) and the response data (`factory_output` fields) into the `devices.provision_data` JSONB column. See [Cloud Provision Data](#cloud-provision-data) for details.
...
The cloud maintains a flat snapshot of all provisioning data in the `devices.provision_data` JSONB column. This column is a **superset** of all data from both factory and consumer provisioning events. Whenever a provisioning event occurs (factory or consumer), the relevant keys must be **merged** into the existing `provision_data` — not replaced wholesale.

| Event | Data merged into `provision_data` |
|-------|-----------------------------------|
| Factory provisioning | `factory_input` params sent to device + `factory_output` data returned by device |
| Consumer provisioning | `consumer_input` params sent to device + `consumer_output` data returned by device |

Example after factory provisioning of a Hub with Thread Border Router capability:
```json
{
  "cloud_url": "https://xxx.supabase.co",
  "cloud_anon_key": "eyJ...",
  "thread_credentials": {
    "pan_id": 21334,
    "channel": 15,
    "network_key": "a1b2c3d4...",
    "network_name": "SaturdayVinyl"
  }
}
```

After subsequent consumer provisioning adds WiFi credentials:
```json
{
  "cloud_url": "https://xxx.supabase.co",
  "cloud_anon_key": "eyJ...",
...
|---------|------|---------|
| 1.2.5 | 2026-02-04 | Added Cloud Provision Data section documenting `devices.provision_data` JSONB column; provisioning input and output data must be merged into this column after each provisioning event |
